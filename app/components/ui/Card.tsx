...
const Card: React.FC<SensorCardProps> = ({
  ...
  // Keep peak-to-peak history
  const [peakHistory, setPeakHistory] = React.useState<
  ...
  React.useEffect(() => {
    if (
      ...
      const latestValue = recentPeakToPeakData[recentPeakToPeakData.length - 1];
